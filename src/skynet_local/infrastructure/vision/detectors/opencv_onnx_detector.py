...
class OpenCvOnnxFaceDetector(FaceDetectorBase):
    ...
    detector_yn: cv2.FaceDetectorYN
    recognition_service: FaceRecognizerPort | None
    score_threshold: float

    def __init__(
        self,
        detector_yn: cv2.FaceDetectorYN,
        recognition_service: FaceRecognizerPort | None = None,
        score_threshold: float = 0.6,
        smooth_alpha: float = 0.25,
        expand_ratio: float = 0.25,
    ) -> None:
        super().__init__(smooth_alpha=smooth_alpha, expand_ratio=expand_ratio)
    ...
    def detect(self, frame: np.ndarray) -> list[FaceObservation]:
        ...
        raw_faces_by_track: dict[str, np.ndarray] = {}
        ...
        for i, face_row in enumerate(faces):
            ...
            visual_box = self.expand_box(
                *smoothed_box, frame_shape=frame.shape, ratio=self.expand_ratio
            )
            ...
            prompt: str | None = "Register face / switch GUI mode / enable diagnostics"
